...
app aka the :ref:`appui`. At the heart of the app's logic is the
:ref:`knutclient` which is used by each service to communicate to the
server. Each service in the server can be accessed via a dedicated API to which
the :ref:`knutclient` sends a JSON formatted message string. Read the `API
documentations
